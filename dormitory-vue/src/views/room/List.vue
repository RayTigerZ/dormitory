<template>
	<div>
		<div class="buildind-div">
			<el-button v-for="item in buildings" class="building-btn" :key="item.id" :plain="item.id != search.buildingId" type="success" @click="search.buildingId = item.id">
				{{ item.name }}
			</el-button>
		</div>
		<div>
			<el-button type="primary" icon="el-icon-plus" @click="buildingDialog.visible = true">增加楼宇</el-button>
			<el-button type="primary" icon="el-icon-plus" @click="roomDialog.visible = true">增加宿舍</el-button>
			<el-button type="primary" icon="el-icon-plus" @click="roomBatchDialog.visible = true">批量导入宿舍</el-button>
		</div>
			...
			<el-col :span="4">
				...
				<div class="floor-body">
					<template v-for="floor in floors">
						<div :key="floor.num" class="floor" @click="search.floor = floor.num" :class="{ active: floor.num == search.floor }">
							...
							<el-collapse-item :key="room.number" :name="room.number">
								...
								<div class="beds">
									<template v-for="index in room.size">
										<div :key="index" class="bed" :class="{ 'bed-lived': room.students.length > index }">
		...
		<!-- 弹出窗：增加楼宇 -->
		<el-dialog :visible.sync="buildingDialog.visible" width="40%" title="增加楼宇">
			<el-form :model="buildingDialog.building" label-position="left" label-width="80px" ref="buildingForm" :rules="buildingRules">
				<el-form-item label="楼宇名称" prop="name">
					<el-input type="text" v-model="buildingDialog.building.name"></el-input>
				</el-form-item>
			</el-form>
		...
		<el-dialog :visible.sync="roomDialog.visible" width="30%" title="增加宿舍">
			<el-form :model="roomDialog.room" ref="roomForm" label-width="80px" label-position="left" :rules="roomRules">
				<el-form-item label="宿舍楼" prop="buildingId">
					<el-input type="text" v-model.number="roomDialog.room.buildingId">
				...
				<el-form-item label="最大可住" prop="size">
					<el-input type="text" v-model.number="roomDialog.room.size"></el-input>
				</el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">
		...
		<!-- 弹出窗：批量导入宿舍 -->
		<el-dialog :visible.sync="roomBatchDialog.visible" width="40%" title="批量导入宿舍">
			<el-form>
				<el-form-item>
					<el-button type="primary" @click="download()" icon="el-icon-download">下载批量导入模板</el-button>
				</el-form-item>
				<el-form-item>
					<el-upload ref="upload" action="" :auto-upload="false" :limit="1" :http-request="uploadBatch">
...
import { getRooms, uploadBatch, saveRoom } from '@/api/room'
...
export default {
	...
	methods: {
		getBuildings() {
			...
			getBuildings({ ...data }).then(res => {
				this.buildings = res.data.data.records
	...
	watch: {
		...
		'roomDialog.visible'(newVal) {
			if (newVal == false) {
				this.$refs['roomForm'].resetFields()
			}
		},
		'roomDialog.room.buildingId'(newVal) {
			let str
			if (newVal >= 10) {
				str = newVal.toString()
			} else {
				str = '0' + newVal.toString()
			}

			this.roomDialog.room.number = str
		},
		'search.buildingId'() {
			this.getFloors()
			this.getRooms()
